...
impl DnsRecord {
    ...
    pub fn write(&self, buffer: &mut ByteBuffer) -> Result<usize> {
...
impl DnsPacket {
    ...
    pub fn write(&mut self, buffer: &mut ByteBuffer) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    // For now, queries and handled sequentially, so an infinite loop for servicing
